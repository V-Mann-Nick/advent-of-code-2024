...
const Alloactor = std.mem.Allocator;
const print = std.debug.print;
const assert = std.debug.assert;

const PageSet = std.AutoHashMap(u8, void);
const Rules = std.AutoHashMap(u8, PageSet);
const Update = std.ArrayList(u8);
...
pub fn main() !void {
    ...
    var rules, var updates = try parseInput(allocator);
    defer {
        var value_iterator = rules.valueIterator();
        while (value_iterator.next()) |v| {
            v.*.deinit();
        }
        rules.deinit();
...
fn solvePart1(rules: *const Rules, updates: *const Updates) u32 {
    var total: u32 = 0;

    for (updates.items) |update| {
        if (!isCorrectlyOrdered(&update, rules)) continue;
        total += getMiddleNumber(update.items);
    }

    return total;
}

fn solvePart2(rules: *const Rules, updates: *Updates) u32 {
    var total: u32 = 0;

    for (updates.items) |update| {
        if (isCorrectlyOrdered(&update, rules)) continue;
        std.mem.sort(u8, update.items, rules, isPageBefore);
        assert(isCorrectlyOrdered(&update, rules));
        total += getMiddleNumber(update.items);
    }
...
fn isPageBefore(rules: *const Rules, lhs: u8, rhs: u8) bool {
    return rules.get(lhs).?.contains(rhs);
}

fn getMiddleNumber(pages: []const u8) u8 {
    const middle_idx = @divFloor(pages.len, 2);
    return pages[middle_idx];
}

fn isCorrectlyOrdered(update: *const Update, rules: *const Rules) bool {
    for (update.items, 0..) |page, i| {
        const later_pages = update.items[(i + 1)..];
        const after_rules = rules.get(page).?;
        for (later_pages) |later_page| {
            if (!after_rules.contains(later_page)) {
                return false;
            }
        }
    }
...
fn parseInput(allocator: Alloactor) !struct { Rules, Updates } {
    var rules_and_updates = std.mem.splitSequence(u8, input, "\n\n");
    return .{
        try parseRules(allocator, rules_and_updates.next().?),
        try parseUpdates(allocator, rules_and_updates.next().?),
    };
}

fn parseRules(allocator: Alloactor, rules_str: []const u8) !Rules {
    var rules = Rules.init(allocator);

    var rule_str_iterator = std.mem.splitSequence(u8, rules_str, "\n");
    while (rule_str_iterator.next()) |rule| {
        var before_and_after = std.mem.splitSequence(u8, rule, "|");

        const before_str = before_and_after.next().?;
        const before = try std.fmt.parseInt(u8, before_str, 10);

        const after_str = before_and_after.next().?;
        const after = try std.fmt.parseInt(u8, after_str, 10);

        const result = try rules.getOrPut(before);
        if (!result.found_existing) {
            result.value_ptr.* = PageSet.init(allocator);
        }
        try result.value_ptr.*.put(after, {});
    }

    return rules;
}

fn parseUpdates(allocator: Alloactor, updates_str: []const u8) !Updates {
    var updates = Updates.init(allocator);

    var updates_str_iterator = std.mem.split(u8, updates_str, "\n");
    while (updates_str_iterator.next()) |update_str| {
        var update = Update.init(allocator);

        var page_str_iterator = std.mem.split(u8, update_str, ",");
        while (page_str_iterator.next()) |page_str| {
