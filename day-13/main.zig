...
pub fn main() !void {
    ...
    var tokens_part1: i64 = 0;
    var tokens_part2: i64 = 0;

    var claw_machine_iterator = ClawMachineIterator.init();
    while (claw_machine_iterator.next()) |claw_machine| {
        const x1 = claw_machine.x1.parse();
        const x2 = claw_machine.x2.parse();
        const xt = claw_machine.xt.parse();
        const y1 = claw_machine.y1.parse();
        const y2 = claw_machine.y2.parse();
        const yt = claw_machine.yt.parse();

        tokens_part1 += blk: {
            const b = try solveB(x1, x2, xt, y1, y2, yt) orelse break :blk 0;
            const a = try solveA(x1, x2, xt, b) orelse break :blk 0;
            break :blk a * 3 + b;
        };

        tokens_part2 += blk: {
            const xt_new = xt + 10000000000000;
            const yt_new = yt + 10000000000000;
            const b = try solveB(x1, x2, xt_new, y1, y2, yt_new) orelse break :blk 0;
            const a = try solveA(x1, x2, xt_new, b) orelse break :blk 0;
            break :blk a * 3 + b;
        };
    }
...
const LazyNumber = struct {
    n_str: []const u8,

    fn parse(self: *const LazyNumber) i64 {
        var n: i64 = 0;
        for (self.n_str) |char| {
            n = n * 10 + char - 48;
        }
...
const ClawMachineIterator = struct {
    block_iterator: std.mem.SplitIterator(u8, .sequence),

    fn init() ClawMachineIterator {
        const block_iterator = std.mem.splitSequence(u8, input, "\n\n");
        return ClawMachineIterator{ .block_iterator = block_iterator };
    }

    fn getAfterChar(part: []const u8, char: u8) LazyNumber {
        var sub_parts_iterator = std.mem.split(u8, part, &[_]u8{char});
        _ = sub_parts_iterator.next().?;
        const n_str = sub_parts_iterator.next().?;
        return LazyNumber{ .n_str = n_str };
    }

    fn parseLine(line: []const u8, char_before_number: u8) [2]LazyNumber {
        var button_line_iterator = std.mem.splitSequence(u8, line, ",");

        const before_comma = button_line_iterator.next().?;
        const x = getAfterChar(before_comma, char_before_number);

        const after_comma = button_line_iterator.next().?;
        const y = getAfterChar(after_comma, char_before_number);

        return .{ x, y };
    }

    fn next(self: *ClawMachineIterator) ?ClawMachine {
        const block = self.block_iterator.next() orelse return null;
        var line_iterator = std.mem.splitSequence(u8, block, "\n");

        const x1, const y1 = parseLine(line_iterator.next().?, '+');
        const x2, const y2 = parseLine(line_iterator.next().?, '+');
        const xt, const yt = parseLine(line_iterator.next().?, '=');
